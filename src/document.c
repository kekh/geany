...
gboolean document_save_file(GeanyDocument *doc, gboolean force)
{
	...
	if (! main_status.quitting)
	{
		...
		vte_cwd((doc->real_path != NULL) ? doc->real_path : doc->file_name, FALSE);
#endif
		basename = document_get_basename_for_display(doc, -1);
		g_object_set(page, "label", basename, "icon", doc->file_type->icon, NULL);
		g_free(basename);
	}
	g_free(locale_filename);
...
GPtrArray *document_array_sorted_copy(GCompareFunc fn)
{
	...
	/* Copy only valid ones, the sort func probably can't deal with invalid docs */
	foreach_document(i)
		g_ptr_array_add(array_copy, documents[i]);
